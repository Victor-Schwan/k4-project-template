import sys
from itertools import zip_longest
from math import sqrt
from pathlib import Path

from podio.root_io import Reader
from tabulate import tabulate

sys.path.insert(0, str(Path(__file__).resolve().parent))

from config_track_merger import (
    CLU_TRACK_COLL_NAME,
    MCP_COLL_NAME,
    PATHS,
    SI_TRACK_COLL_NAME,
    TRACK_VARIATIONS,
)

# Derive the collection names we care about from the config
_GREEDY = TRACK_VARIATIONS["Greedy"]["collections"]
_AMBIG = TRACK_VARIATIONS["Ambiguous"]["collections"]
_CLU_W_SI = TRACK_VARIATIONS["CluWithSi"]["collections"]

GREEDY_CAND_COLL = _GREEDY["merge_candidates"]
GREEDY_REFT_COLL = _GREEDY["refit_output"]
AMBIG_REFT_COLL = _AMBIG["refit_output"]
CLU_W_SI_COLL = _CLU_W_SI["merge_candidates"]
CLU_W_SI_REFT_COLL = _CLU_W_SI["refit_output"]

INPUT_FILE_2_ANA = PATHS["output"]


def main():
    if not Path(INPUT_FILE_2_ANA).exists():
        print(f"Error: Output file {INPUT_FILE_2_ANA} not found.")
        return

    events = Reader(str(INPUT_FILE_2_ANA)).get("events")

    coll_names = [
        MCP_COLL_NAME,
        GREEDY_REFT_COLL,
        AMBIG_REFT_COLL,
        CLU_W_SI_REFT_COLL,
        CLU_W_SI_COLL,
        SI_TRACK_COLL_NAME,
        CLU_TRACK_COLL_NAME,
        GREEDY_CAND_COLL,
    ]

    for i, event in enumerate(events):
        print(f"\n{'#' * 80}\n# Processing Event {i:3} \n{'#' * 80}")

        collections_data = []
        for name in coll_names:
            try:
                collections_data.append(event.get(name))
            except KeyError:
                print(f"[!] Warning: Collection {name} missing in event {i}")

        lengths = [len(c) if c is not None else 0 for c in collections_data]
        if len(set(lengths)) > 1:
            print(
                "Note: Collections have different sizes: "
                + ", ".join([f"{n}: {l}" for n, l in zip(coll_names, lengths)])
            )

        table_data = []
        headers = [
            "Coll",
            "ID",
            "Chi2/NDF",
            "Hits",
            "D0",
            "Z0",
            "Phi",
            "Omega",
            "TanL",
        ]

        for i, tracks_group in enumerate(
            zip_longest(*collections_data, fillvalue=None)
        ):
            if i > 0:
                # Add a separator line between groups of i-th tracks for clarity
                table_data.append(["~" * 5] * len(headers))

            for idx, track in enumerate(tracks_group):
                coll_label = coll_names[idx]

                if track is None:
                    # Fill row with '-' if the collection is shorter than others
                    table_data.append([coll_label] + ["-"] * (len(headers) - 1))
                    continue

                if coll_label == MCP_COLL_NAME:
                    if track.getGeneratorStatus() != 1:
                        # Only show stable particles
                        continue
                    p_t = sqrt(track.getMomentum().x ** 2 + track.getMomentum().y ** 2)
                    a = 3e-4
                    B_z = 2.0
                    table_data.append(
                        [
                            coll_label,
                            track.getObjectID().index,
                            "---",
                            "---",
                            f"{sqrt(track.getVertex().x ** 2 + track.getVertex().y ** 2):.2f}",
                            f"{track.getVertex().z:.2f}",
                            "---",
                            f"|{a * B_z / p_t:.4e}|",
                            f"{track.getMomentum().z / p_t:.4f}",
                        ]
                    )
                    continue

                ndf = track.getNdf()
                chi2_str = f"{track.getChi2():.2f}/{ndf}"
                if ndf > 0:
                    chi2_str += f"={track.getChi2() / ndf:.2f}"

                if track.trackStates_size() == 0:
                    table_data.append(
                        [
                            coll_label,
                            track.getObjectID().index,
                            chi2_str,
                            track.trackerHits_size(),
                            "NO STATE",
                            "---",
                            "---",
                            "---",
                            "---",
                        ]
                    )
                else:
                    state = track.getTrackStates(0)
                    table_data.append(
                        [
                            coll_label,
                            track.getObjectID().index,
                            chi2_str,
                            track.trackerHits_size(),
                            f"{state.D0:.4f}",
                            f"{state.Z0:.4f}",
                            f"{state.phi:.4f}",
                            f"{state.omega:.4e}",
                            f"{state.tanLambda:.4f}",
                        ]
                    )

        print(tabulate(table_data, headers=headers, tablefmt="simple_outline"))


main()
